...
class SiteHelper with ChangeNotifier {
  ...
  /// Polygon-download request keys currently in flight for each site, one per device
  ...
  /// set becomes empty, so `containsKey` / `isEmpty` style checks reflect "nothing in flight".
  ///
  ...
  /// devices were still downloading. Exact request keys also make cancellation generation-safe:
  /// a late completion from an invalidated generation cannot decrement a newer generation's
  /// count for the same site.
  static Map<Site, Set<Object>> siteDownloadSinceLastClick = <Site, Set<Object>>{};

  /// True if [site] currently has at least one polygon-download chain in flight.
  static bool isSiteDownloadInFlight(Site site) =>
      siteDownloadSinceLastClick[site]?.isNotEmpty ?? false;
  ...
  static void startSiteDownload(Site site, Object requestKey) {
    siteDownloadSinceLastClick.putIfAbsent(site, () => <Object>{}).add(requestKey);
  }
  ...
  static void finishSiteDownload(Site site, Object requestKey) {
    final Set<Object>? requests = siteDownloadSinceLastClick[site];
